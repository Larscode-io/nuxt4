  ...
  >
    <form>
      ...
      >
        ...
        >
          <v-radio
            v-for="type of searchTypesForJudgment"
            :key="type.id"
            :label="type.label"
            :value="type.id"
            name="type"
      ...
      <ValidationProvider
        ...
        v-slot="{ errors }"
        :key="
          toNameKey(t('general.message.jurisdisctionArea'))
        "
        :name="
          toNameKey(t('general.message.jurisdisctionArea'))
        "
      ...
      <ValidationProvider
        ...
        class="d-flex"
        :name="
          toNameKey(t('general.message.referralDecision'))
        "
      >
        <v-text-field
      ...
      <ValidationProvider
        ...
        :key="
          toNameKey(
            t('general.message.dateOfBeOfficialJournal'),
          )
        "
        class="d-flex"
        :name="
      ...
      >
        <v-text-field
          ...
          hint="DD/MM/YYYY"
          :label="
            t('general.message.dateOfBeOfficialJournalStart')
          "
          persistent-hint
          :error-messages="errors"
        />
        ...
        <v-text-field
          ...
          hint="DD/MM/YYYY"
          :label="
            t('general.message.dateOfBeOfficialJournalEnd')
          "
          persistent-hint
          :error-messages="errors"
        />
...
const applicantTypes = computed(() => [])
const jurisdiction = computed(() => undefined)
const jurisdictionList = computed(() => [])
const jurisdictionArea = computed(() => undefined)
const jurisdictionAreaList = computed(() => [])
  ...
  if (isSearchJudgmentsTypeRollNumber.value) {
    return (
      ...
      && (!payload?.judgmentTypes
        || payload?.judgmentTypes?.length === 0)
      && (!payload?.judgmentDates
        || payload?.judgmentDates?.length === 0)
      && !payload?.rollNumber
...
const formattedSearchResult = computed(() => {
  ...
  const groupedByFirstLevelTitle = groupBy(
    searchResponse,
    'firstLevelTitle',
  )
...
const hasResults = computed(() => formattedSearchResult.value && formattedSearchResult.value.length > 0)

const rollNumber = computed(() => '123456')
const judgmentTypes = computed(() => [] as JudgmentType[])
const judgmentTypesFiltered = (() => {
  if (isSearchJudgmentsTypeRollNumber.value) {
...
const type = computed(() => SearchJudgmentsType.rollNumber)
const searchTypesForJudgment = computed(() => [
  {
