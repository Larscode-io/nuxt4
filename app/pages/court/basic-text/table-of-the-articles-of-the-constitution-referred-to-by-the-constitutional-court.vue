...
// Debouncing
// De API-call pas wordt uitgevoerd na een korte pauze bij wijzigingen
...
const { data, pending } = useAsyncData<DataResponse>(
  () => `${locale.value}-articles-${debouncedPage.value}-${debouncedPerPage.value}`,
  () => $fetch<DataResponse>(`${baseURL}${ApiUrl.arcticlesConstReferredByConstCourt}`, {
    ...
    watch: [debouncedPage, debouncedPerPage, locale],
    default: () => ({
      total: 0,
      rows: [],
    }),
