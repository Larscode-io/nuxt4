<!-- API based page -->
<script setup lang="ts">
import { ref, computed, onMounted } from 'vue'
import img from '~/assets/img/banner-judgment.png'
import { useLanguage } from '@/composables/useLanguage'
import BannerImage from '~/components/BannerImage.vue'
// import ErrorCard from '~/components/ErrorCard.vue'
// import EmptyComponent from '~/components/EmptyComponent.vue'
import { ApiUrl, ContentKeys } from '~/core/constants'

const { t, locale } = useLanguage()

const config = useRuntimeConfig()
const baseURL = config.public.apiBaseUrl

const questions = ref(null)
const i18nKeys = ref({})

// during development, if the apiBaseUrl is not set in .env, the legacy server URL node04 will be used (nuxt.config.ts).
const { data, error } = useLazyFetch(`${baseURL}${ApiUrl.questionsReferred}?lang=${locale.value}`)
if (error.value) {
  console.error(error.value)
}

// const { data, error } = await useFetch(`${ApiUrl.questionsReferred}?lang=${locale.value}`).json()

// if (error.value) {
//   throw error.value
// }

// questions.value = data.value

// useHead({
//   title: t(i18nKeys.value.menu.decisions.preliminaryRulingsFromTheCourtOfJusticeOfTheEuTitle),
//   meta: [
//     {
//       hid: 'description',
//       name: 'description',
//       content: t(i18nKeys.value.menu.decisions.preliminaryRulingsFromTheCourtOfJusticeOfTheEuTitleDescription) || '',
//     },
//   ],
// })

// const hasQuestion = computed(() => questions.value?.length > 0)
</script>

<template>
  <div>
    <BannerImage
      :title="t('menu.decisions.preliminary-rulings-from-the-court-of-justice-of-the-eu')"
      :description="t('menu.decisions.preliminary-rulings-from-the-court-of-justice-of-the-eu-title-description')"
      :image="img"
      alt="alt.banner.courtroom"
    />
  </div>
  {{ data }}
  <!-- <v-container class="flex-column align-start flex-nowrap" fluid>
      <v-row v-if="$fetchState.pending" class="d-flex" align="flex-start" justify="center">
        <div class="col-12 col-md-9">
          <v-skeleton-loader class="mx-auto" max-width="300" type="article" />
        </div>
      </v-row>
      <v-row v-else-if="$fetchState.error" class="d-flex" align="flex-start" justify="center">
        <div class="col-12 col-md-12">
          <ErrorCard :message="$t($i18nKeys.error.fetchingData)" :show-go-home="false" />
        </div>
      </v-row>
      <v-row v-else class="d-flex" align="flex-start" justify="center">
        <div class="col-12 col-md-3">
          <div class="my-4" :aria-label="$tc(i18nKeys.aria.label.print)" @click="print('list')">
            <v-btn text>
              <v-icon left small block>
                mdi-printer
              </v-icon>
              {{ $t($i18nKeys.general.message.printList) }}
            </v-btn>
          </div>
        </div>
        <div v-if="hasQuestion" ref="list" class="col-12 col-md-9">
          <v-card v-for="question of questions" :id="question.id" :key="question.id" class="mx-auto my-4" outlined>
            <v-card-title>
              <h3 style="font-size:1rem">
                {{ question.sortId }}. {{ question.title }}
              </h3>
            </v-card-title>
            <v-card-text>
              <ul>
                <li>
                  {{ $t($i18nKeys.general.message.referringJudgmentConstCourt) }}
                  {{ $t($i18nKeys.general.message.colon) }}
                  <a :href="question.referringJudgmentOfTheConstitutionalCourtLink">
                    {{ question.referringJudgmentNumberOfTheConstitutionalCourt }}
                  </a>
                  (
                  {{
                  `${$tc($i18nKeys.general.message.rollNumber)}${$t(
                  $i18nKeys.general.message.colon,
                  )} ${
                  question.referringJudgmentOfTheConstitutionalCourtIds[0]
                  } `
                  }})
                  {{ $t($i18nKeys.general.of) }}
                  {{
                  question.formatedReferringJudgmentDateOfTheConstitutionalCourt
                  }}
                </li>
                <li v-if="question.noticeInTheOfficialJournalLink">
                  <a :href="question.noticeInTheOfficialJournalLink">
                    {{ $t($i18nKeys.general.message.noticeOfficialJournal) }}
                    {{ $t($i18nKeys.general.of) }}
                    {{ question.formatedNoticeDateInTheOfficialJournal }}
                  </a>
                  <span v-if="question.noticeInTheOfficialJournalId">
                    (
                    {{
                    `${$tc($i18nKeys.general.message.case).toLowerCase()}${$t(
                    $i18nKeys.general.message.colon,
                    )} ${question.noticeInTheOfficialJournalId} `
                    }})
                  </span>
                </li>
                <li v-else>
                  {{ $t($i18nKeys.error.noticeOfficialJournalNotAvailable) }}
                </li>
                <li v-if="question.opinionOfTheAdvocateGeneralLink">
                  <a :href="question.opinionOfTheAdvocateGeneralLink">
                    {{ $t($i18nKeys.general.message.opinionAdvocateGeneral) }}
                    {{ $t($i18nKeys.general.of) }}
                    {{ question.formatedOpinionDateOfTheAdvocateGeneral }}
                  </a>
                </li>
                <li v-else-if="question.opinionOfTheAdvocateGeneralLink === ''">
                  {{ $t($i18nKeys.error.opinionAdvocateGeneralNotPublished) }}
                </li>
                <li v-else>
                  {{ $t($i18nKeys.error.opinionAdvocateGeneralNotAvailable) }}
                </li>

                <li v-if="question.judgmentOfTheCourtOfJusticeLink">
                  <a :href="question.judgmentOfTheCourtOfJusticeLink">
                    {{ $t($i18nKeys.general.message.judgmentCourtJustice) }}
                    ({{ question.judgmentNumberOfTheCourtOfJustice }})
                    {{ $t($i18nKeys.general.of) }}
                    {{ question.formatedJudgmentDateOfTheCourtOfJustice }}
                  </a>
                </li>
                <li v-else>
                  {{ $t($i18nKeys.error.judgmentCourtJusticeNotDelivered) }}
                </li>
                <li v-if="question.judgmentNumberOfTheConstitutionalCourt" class="green--text">
                  {{ $t($i18nKeys.general.message.judgmentConstCourt) }}
                  {{ $t($i18nKeys.general.message.colon) }}
                  <a v-if="question.judgmentOfTheConstitutionalCourtPath"
                    :href="question.judgmentOfTheConstitutionalCourtPath">{{
                    question.judgmentNumberOfTheConstitutionalCourt }}</a>
                  <span v-else>{{
                    question.judgmentNumberOfTheConstitutionalCourt
                    }}</span>
                  {{ $t($i18nKeys.general.of) }}
                  {{ question.formatedjudgmentDateOfTheConstitutionalCourt }}
                </li>
                <li v-else class="red--text">
                  {{ $t($i18nKeys.error.judgmentCourtCourtNotDelivered) }}
                </li>
              </ul>
            </v-card-text>
          </v-card>
        </div>
        <div v-else ref="list" class="col-12 col-md-9">
          <EmptyComponent />
        </div>
      </v-row>
    </v-container> -->
</template>

<style lang="scss" scoped>
.container {
  padding: 0 !important;
  @include mobile {
    padding: 32px;
  }
}
.d-flex {
  max-width: 1260px !important;
  margin: auto;
  margin-bottom: 80px;
  @include mobile {
    margin-bottom: 40px;
    width: 100%;
  }
}
.v-card__title {
  font-size: 1rem;
  line-height: 24px;
  word-break: break-word;
}

.v-application .green--text{
  color: #1B5E20 !important;
}

.v-application .red--text{
  color:#c32a3a !important;
}

a{
  text-decoration: none;

}
</style>
