<template>
  <div :id="id" class="search-result-card">
    <a class="link-pdf pdf-content no-decoration" :href="judgmentFilePath" rel="noopener noreferrer" target="_blank"
      ...
      <v-icon size="24" class="mr-4" color="red" left>
        mdi-file-pdf-box
      </v-icon>
      <div class="content">
        ...
        <div v-if="rollNumbers && rollNumbers.length > 0" class="row-to-column">
          <p class="titleSearchResult">{{ t('general.message.roll-number') }}</p>
          <p>{{ rollNumbers.join(' - ') }}</p>
        </div>
        <div v-if="judgmentTypes && judgmentTypes.length > 0" class="row-to-column">
          <p class="titleSearchResult">{{ t('general.message.judgment-type') }}</p>
          <ul>
            <li v-for="(type, index) in judgmentTypes" :key="index">
              {{ type }}
            </li>
          </ul>
        </div>
        <div v-if="dateOfBeOfficialJournal" class="row-to-column">
          <p class="titleSearchResult">{{ t('general.message.date-of-be-official-journal') }}</p>
          <p>{{ dateOfBeOfficialJournal || emptyValue }}</p>
        </div>
        <div v-if="belgianOfficialPage" class="row-to-column">
