<template>
    <v-card class="member-card" :class="{ 'member-card-hoover': showDetails, 'small-card': isSmall }" :width="width"
        ...
        <v-img :src="resolvedSrc" alt ="Member image" </v-img>
...
const { getImageUrlx } = useImageMap()

watchEffect(() => {
    resolvedSrc.value = props.image && props.image !== 'undefined' ? getImageUrlx(`members/${props.image}`) : ''
})
